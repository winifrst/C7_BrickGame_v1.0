 ...
 * state and logic, including the tetramino types, field dimensions, and
...
/**
 * @brief Enum representing the different types of tetraminos.
 */
typedef enum {
  O_tetramino = 1,
  I_tetramino,
 ...
 * tetraminos, moving them, attaching them to the field, pausing, and the game
 * over state.
 */
typedef enum {
  Start_state,
 ...
 * tetramino, game field, score, level, and more.
 */
typedef struct {
  FiniteState_t state;
  ...
  int **field_base;
  int **next_tetramino;
  TetraminoType_t next_type;
  int **current_tetramino;
  int **collision_test_tetramino;
  TetraminoType_t current_type;
  int x_position;
  int y_position;
  int score;
  int high_score;
